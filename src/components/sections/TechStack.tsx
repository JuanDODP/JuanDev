    ...
    <section ref={sectionRef} id="tech" className="section" style={{ background: 'linear-gradient(180deg, var(--bg) 0%, var(--bg-2) 100%)' }}>
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '1rem' }} className="bento-grid">
          ...
          <div data-reveal style={{ gridColumn: 'span 5', gridRow: 'span 2', padding: '2rem', background: 'var(--glass)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', minHeight: '340px', transition: 'border-color var(--t-base)', cursor: 'default' }}
          ...
          <div data-reveal style={{ gridColumn: 'span 7', padding: '1.75rem', background: 'var(--glass)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', position: 'relative', overflow: 'hidden', transition: 'border-color var(--t-base)', cursor: 'default' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--border-2)'; }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'; }}
          >
            {/* Cyan top accent */}
          ...
          {/* Cloud & Infra */}
          <div data-reveal style={{ gridColumn: 'span 4', padding: '1.5rem', background: 'var(--glass)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', position: 'relative', transition: 'border-color var(--t-base)', cursor: 'default' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--border-2)'; }}
            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'; }}
          >
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-2)', lineHeight: 1.65, marginBottom: '0.75rem' }}>Infraestructura AWS de alta disponibilidad con pipelines CI/CD automatizados.</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: '#fbbf24', fontWeight: 700 }}>AWS S3</span>
              <div style={{ flex: 1, height: 3, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ width: '100%', height: '100%', background: 'linear-gradient(90deg, #fbbf24, #f59e0b)' }} />
              </div>
            </div>
            {tags(['AWS', 'EC2', 'Docker', 'GitHub Actions', 'Nginx'])}
          </div>

          {/* Innovación */}
          <div data-reveal style={{ gridColumn: 'span 3', padding: '1.5rem', background: 'var(--glass)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', position: 'relative', transition: 'border-color var(--t-base)', cursor: 'default' }}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(129,140,248,0.4)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 0 30px rgba(129,140,248,0.1)'; }}
          ...
          >
            ...
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', fontWeight: 700, color: 'var(--text-1)', marginBottom: '0.4rem' }}>Innovación</h3>
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-2)', lineHeight: 1.65 }}>Integración avanzada de OpenAI APIs y LLMs personalizados para automatización de procesos.</p>
          </div>
